...
class AwsSemaphoreAgentStack extends Stack {
  ...
  createLaunchTemplate(ssmParameter, instanceRole, securityGroup) {
    ...
    if (this.argumentStore.get("SEMAPHORE_AGENT_USE_IPV6") !== "true") {
      ...
      // When a network interface is specified, the security group can not also be set in the launch template props.
      // Therefore, we only set the security group here, if it is not configured for IPv6.
      launchTemplateProps.securityGroup = securityGroup;
    }

    let launchTemplate = new LaunchTemplate(this, 'launchTemplate', launchTemplateProps);

    if (this.argumentStore.get("SEMAPHORE_AGENT_USE_IPV6") === "true") {
      // It doesn't look like it in this configuration, but the resulting EC2 instances will also continue to have IPv4 enabled.
      launchTemplate.node.defaultChild.launchTemplateData.networkInterfaces = [
        {
          deviceIndex: 0,
  ...
  createAutoScalingGroup(launchTemplate) {
    ...
    if (!this.argumentStore.isEmpty("SEMAPHORE_AGENT_ASG_INSTANCE_REQUIREMENTS_JSON")) {
      const instanceRequirements = JSON.parse(this.argumentStore.get("SEMAPHORE_AGENT_ASG_INSTANCE_REQUIREMENTS_JSON"));
